...
class AgentPublicKey(Structure):
    pass
...
class LibSsh2:
    ...
    def __init__(self):
        ...
        self.session_init_ex = self.setup('session_init_ex', [c_void_p, c_void_p, c_void_p, c_void_p], POINTER(Session))
        ...
        self.session_disconnect_ex = self.setup('session_disconnect_ex', [POINTER(Session), c_int, c_char_p, c_char_p], c_int)
        ...
        self.session_free = self.setup('session_free', [POINTER(Session)], c_int)
        #int libssh2_session_banner_set(LIBSSH2_SESSION *session, const char *banner);
        self.session_banner_set = self.setup('banner_set', [POINTER(Session), c_char_p], c_int)
        #void libssh2_trace(LIBSSH2_SESSION *session, int bitmask);
        self.trace_set = self.setup('trace', [POINTER(Session), c_int], None)
        #int libssh2_session_startup(LIBSSH2_SESSION *session, int socket);
        self.session_startup = self.setup('session_startup', [POINTER(Session), c_int], c_int)
        #void libssh2_session_set_blocking(LIBSSH2_SESSION *session, int blocking);
        self.session_set_blocking = self.setup('session_set_blocking', [POINTER(Session), c_int], None)
        
        #========== LIBSSH2_USERAUTH
        #char * libssh2_userauth_list(LIBSSH2_SESSION *session, const char *username, unsigned int username_len);
        self.userauth_list = self.setup('userauth_list', [POINTER(Session), c_char_p, c_uint], c_char_p)
        ...
        self.userauth_password_ex = self.setup('userauth_password_ex', [POINTER(Session), c_char_p, c_uint, c_char_p, c_uint, c_void_p], c_int)
        ...
        self.channel_open_ex = self.setup('channel_open_ex', [POINTER(Session), c_char_p, c_uint, c_uint, c_uint, c_char_p, c_uint], POINTER(Channel))
        ...
        self.channel_close = self.setup('channel_close', [POINTER(Channel)], c_int)
        #int libssh2_channel_free(LIBSSH2_CHANNEL *channel);
        self.channel_free = self.setup('channel_free', [POINTER(Channel)], c_int)
        ...
        self.channel_process_startup = self.setup('channel_process_startup', [POINTER(Channel), c_char_p, c_uint, c_char_p, c_uint], c_int)
        ...
        self.channel_read_ex = self.setup('channel_read_ex', [POINTER(Channel), c_int, c_char_p, c_size_t], c_ssize_t)
        ...
        self.agent_init = self.setup('agent_init', [POINTER(Session)], POINTER(Agent))
        #int libssh2_agent_connect(LIBSSH2_AGENT *agent);
        self.agent_connect = self.setup('agent_connect', [POINTER(Agent)], c_int)
        #int libssh2_agent_list_identities(LIBSSH2_AGENT *agent);
        self.agent_list_identities = self.setup('agent_list_identities', [POINTER(Agent)], c_int)
        ...
        self.agent_get_identity = self.setup('agent_get_identity', [POINTER(Agent), POINTER(POINTER(AgentPublicKey)), POINTER(AgentPublicKey)], c_int)
        #int libssh2_agent_userauth(LIBSSH2_AGENT *agent,   const char *username,   struct libssh2_agent_publickey *identity);
        self.agent_userauth = self.setup('agent_userauth', [POINTER(Agent), c_char_p, POINTER(AgentPublicKey)], c_int)
        #int libssh2_agent_disconnect(LIBSSH2_AGENT *agent);
        self.agent_disconnect = self.setup('agent_disconnect', [POINTER(Agent)], c_int)
        #void libssh2_agent_free(LIBSSH2_AGENT *agent);
        self.agent_free = self.setup('agent_free', [POINTER(Agent)], None)
